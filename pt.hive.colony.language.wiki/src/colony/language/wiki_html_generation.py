    ...
    def visit_program_node(self, node):
        if self.visit_index == 0:
            ...
            if not self.configuration_map.get(SIMPLE_PARSE_VALUE, False):
                ...
                self._write("<span class=\"menu\"><a href=\"documentation_index.html\">Home</a></span>")
                self._write("<span class=\"menu\">Index</span>")
                self._write("<span class=\"menu\">Contribute</span>")
                self._write("<span class=\"menu\">Credits</span>")
                self._write("</div")
                self._write("</div>")
                self._write("</div>")
