...
class WikiCommandExtension(language_wiki.wiki_extension_system.WikiExtension):
    """
    The wiki command extension class.
    """

    id = "pt.hive.colony.language.wiki.extensions.command"
    """ The extension id """

    name = "Command Generation Plugin"
    """ The name of the extension """

    short_name = "Command Generation"
    """ The short name of the extension """

    description = "Extension for command generation"
    ...
    def generate_html(self, tag_node, visitor):
        ...
        # writes the start span command tag
        string_buffer.write("<span class=\"command\">")

        # processes a new parse in the contents
        visitor.new_parse(contents, CONFIGURATION_MAP, string_buffer)

        # writes the end span command tag
