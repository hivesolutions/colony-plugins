...
class PrintingWin32(colony.System):
    ...
    def print_test(self, printing_options={}):
        ...
        (
            ...
            _printer_size,
            _printer_margins,
        ) = self.get_printer_handler(printing_options)
        ...
        handler_device_context.DrawText(
            TEST_TEXT,
            (0, constants.INCH * -1, constants.INCH * 8, constants.INCH * -2),
            win32con.DT_CENTER,
        )
    ...
    def print_test_image(self, image_path, printing_options={}):
        ...
        real_bitmap_x1 = int(
            (printer_size[0] - printer_margins[0] - real_bitmap_image_width) / 2
        )
        real_bitmap_y1 = int(
            (printer_size[1] - printer_margins[1] - real_bitmap_image_height) / 2
        )
    ...
    def get_printer_handler(self, printing_options):
        ...
        printer_name = printing_options.get(
        ...
        printer_size = printing_options.get(
            "printer_size", self.get_default_printer_size(handler_device_context)
        )

        # retrieves the printable area
        printable_area = printing_options.get(
            "printable_area", self.get_default_printable_area(handler_device_context)
        )

        # retrieves the printer margins
        printer_margins = printing_options.get(
            "printer_margins", self.get_default_printer_margins(handler_device_context)
        )
    ...
    def get_default_printable_area(self, handler_device_context):
        ...
        return handler_device_context.GetDeviceCaps(
            constants.HORIZONTAL_RESOLUTION
        ), handler_device_context.GetDeviceCaps(constants.VERTICAL_RESOLUTION)

    def get_default_printer_size(self, handler_device_context):
        return handler_device_context.GetDeviceCaps(
            constants.PHYSICAL_WIDTH
        ), handler_device_context.GetDeviceCaps(constants.PHYSICAL_HEIGHT)

    def get_default_printer_margins(self, handler_device_context):
        return handler_device_context.GetDeviceCaps(
            constants.PHYSICAL_OFFSET_X
        ), handler_device_context.GetDeviceCaps(constants.PHYSICAL_OFFSET_Y)
