        ...
        if socket_provider:
            ...
            for socket_provider_plugin in socket_provider_plugins:
                ...
                if socket_provider_plugin_provider_name == socket_provider:
                    ...
                    self.abecula_connection = socket_provider_plugin.provide_socket_parameters(parameters)

            # in case the socket was not created, no socket provider found
            if not self.abecula_connection:
            ...
            self.abecula_connection = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

        # sets the socket to be able to reuse the socket
        self.abecula_connection.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

        # binds the abecula socket
        self.abecula_connection.bind((BIND_HOST_VALUE, port))
        ...
        while not self.abecula_connection_close_event.isSet():
            try:
                ...
                while selected_values == ([], [], []):
                    ...
                    # selects the values
                    selected_values = select.select([self.abecula_connection], [], [], CLIENT_CONNECTION_TIMEOUT)
            ...
            try:
                # receives the abecula data from the socket
                data, abecula_address = self.abecula_connection.recvfrom(MESSAGE_MAXIMUM_SIZE)
                ...
                abecula_client_service_task = AbeculaClientServiceTask(self.main_service_abecula_plugin, self.abecula_connection, abecula_address, port, data, service_configuration)
...
class AbeculaClientServiceTask:
    ...
    def __init__(self, main_service_abecula_plugin, abecula_connection, abecula_address, port, data, service_configuration):
        self.main_service_abecula_plugin = main_service_abecula_plugin
    ...
    def start(self):
        ...
        self.current_request_handler(abecula_service_handler_plugins_map)
    ...
    def abecula_request_handler(self, abecula_service_handler_plugins_map):
        # creates the request
        ...
        try:
            ...
            if not handler_name:
                ...
                raise main_serabeculae_abecula_exceptions.AbeculaNoHandlerException("no handler defined for current request")
            ...
            abecula_service_handler_plugin.handle_request(request, handler_arguments)
    ...
    def send_request(self, request):
        ...
        self.abecula_connection.sendto(result, self.abecula_address)
...
class AbeculaRequest:
    ...
    transaction_id = None
    """ The transaction id, identifying a unique abecula request """
    ...
    parameters = {}
    """ The parameters to the abecula request """
    ...
    name_cache_map = {}
    """ Map to be used to cache the name value references in accordance with abecula specification """
    ...
    def _process_answer_data(self, data, current_index, answer_type_integer, answer_data_length):
        """
        Processes the answer data according to the abecula protocol
        specification.
    ...
    def _serialize_query(self, query, current_index):
        """
        Serializes the given query into the abecula binary format.
    ...
    def _serialize_answer(self, answer, current_index):
        """
        Serializes the given answer into the abecula binary format.
    ...
    def _get_name_joined(self, data, current_index):
        """
        Retrieves the name "encoded" according to the abecula
