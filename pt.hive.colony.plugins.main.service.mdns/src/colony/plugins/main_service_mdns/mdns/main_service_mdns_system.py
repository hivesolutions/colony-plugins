...
class MainServiceMdns:
    """
    The main service mdns class.
    """

    main_service_mdns_plugin = None
    """ The main service mdns plugin """

    mdns_service_handler_plugins_map = {}
    """ The mdns service handler plugins map """

    mdns_service = None
    """ The mdns service reference """

    mdns_service_configuration = {}
    """ The mdns service configuration """

    def __init__(self, main_service_mdns_plugin):
        """
        Constructor of the class.

        @type main_service_mdns_plugin: MainServiceMdnsPlugin
        @param main_service_mdns_plugin: The main service mdns plugin.
        """

        self.main_service_mdns_plugin = main_service_mdns_plugin

        self.mdns_service_handler_plugin_map = {}
        self.mdns_service_configuration = {}
    ...
    def start_service(self, parameters):
        ...
        main_service_utils_plugin = self.main_service_mdns_plugin.main_service_utils_plugin

        # generates the parameters
        service_parameters = self._generate_service_parameters(parameters)

        # generates the mdns service using the given service parameters
        self.mdns_service = main_service_utils_plugin.generate_service(service_parameters)

        # starts the mdns service
        self.mdns_service.start_service()
    ...
    def stop_service(self, parameters):
        ...
        # starts the mdns service
        self.mdns_service.stop_service()

    def mdns_service_handler_load(self, mdns_service_handler_plugin):
        # retrieves the plugin handler name
        handler_name = mdns_service_handler_plugin.get_handler_name()

        self.mdns_service_handler_plugins_map[handler_name] = mdns_service_handler_plugin

    def mdns_service_handler_unload(self, mdns_service_handler_plugin):
        # retrieves the plugin handler name
        handler_name = mdns_service_handler_plugin.get_handler_name()

        del self.mdns_service_handler_plugins_map[handler_name]
    ...
    def set_service_configuration_property(self, service_configuration_property):
        ...
        # cleans the mdns service configuration
        colony.libs.map_util.map_clean(self.mdns_service_configuration)

        # copies the service configuration to the mdns service configuration
        colony.libs.map_util.map_copy(service_configuration, self.mdns_service_configuration)
    ...
    def _generate_service_parameters(self, parameters):
        ...
        pool_configuration = {
            "name" : "mdns pool",
            "description" : "pool to support mdns client connections",
        ...
        parameters = {
            ...
            "service_plugin" : self.main_service_mdns_plugin,
            "service_handling_task_class" : MdnsClientServiceHandler,
...
class MdnsClientServiceHandler:
    """
    The mdns client service handler class.
    """
    ...
    def handle_request(self, service_connection):
        # retrieves the mdns service handler plugins map
        mdns_service_handler_plugins_map = self.service_plugin.main_service_mdns.mdns_service_handler_plugins_map

        try:
            # retrieves the request
            request = self.retrieve_request(service_connection)
        except main_service_mdns_exceptions.MainServiceMdnsException:
        ...
        try:
            ...
            if not handler_name:
                # raises an mdns no handler exception
                raise main_service_mdns_exceptions.MdnsNoHandlerException("no handler defined for current request")

            # in case the handler is not found in the handler plugins map
            if not handler_name in mdns_service_handler_plugins_map:
                # raises an mdns handler not found exception
                raise main_service_mdns_exceptions.MdnsHandlerNotFoundException("no handler found for current request: " + handler_name)

            # retrieves the mdns service handler plugin
            mdns_service_handler_plugin = mdns_service_handler_plugins_map[handler_name]
            ...
            except main_service_mdns_exceptions.MdnsRuntimeException, exception:
    ...
    def retrieve_request(self, service_connection):
        ...
        if data_length > MESSAGE_MAXIMUM_SIZE:
            # raises the mdns invalid data exception
            raise main_service_mdns_exceptions.MdnsInvalidDataException("message data overflow")

        # creates a new mdns request
        request = MdnsRequest({})
    ...
    def send_exception(self, service_connection, request, exception):
        ...
        # checks if the error contains a mdns failure mask
        if hasattr(exception, "mdns_failure_mask"):
            # sets the flags out (response) with
            # the mdns failure mask
            request.flags_out |= exception.mdns_failure_mask
    ...
    def send_request(self, service_connection, request):
        ...
        try:
            ...
            # raises the mdns data sending exception
            raise main_service_mdns_exceptions.MdnsDataSendingException("problem sending data")
...
class MdnsRequest:
    ...
    name_cache_map = {}
    """ Map to be used to cache the name value references in accordance with mdns specification """
    ...
    def _serialize_answer(self, answer, current_index):
        """
        Serializes the given answer into the mdns binary format.
