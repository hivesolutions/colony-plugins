...
class MainServiceDns:
    ...
    def start_server(self, socket_provider, port, service_configuration):
        ...
        while not self.dns_connection_close_event.isSet():
            ...
            message, address = self.dns_socket.recvfrom(512)

            print "mensagem de: " + str(address)
            print repr(message)

            HEADER_SIZE = 12

            transaction_id, flags, number_questions, answer_resource_records, authority_resource_records, aditional_resource_records = struct.unpack_from("!HHHHHH", message)

            current_index = HEADER_SIZE

            resource_records_list = []

            for _index in range(number_questions):
                current_record_name_index = current_index

                name_items = []
                name_offset = current_index

                while not message[current_record_name_index] == "\0":
                    partial_length, = struct.unpack_from("!B", message, current_record_name_index)

                    existing_resource = partial_length & 0xC0 == 0xC0

                    if existing_resource:
                        first_byte = partial_length

                        second_byte, = struct.unpack_from("!B", message, current_record_name_index + 1)

                        index = ((first_byte & 0x3F) << 8) + second_byte

                        # ia buscar o valor ao mapa !!!
                    else:
                        name_item = message[current_record_name_index + 1:current_record_name_index + partial_length + 1]

                        name_items.append(name_item)

                        current_record_name_index += partial_length + 1

                resource_record_type, resource_record_class = struct.unpack_from("!HH", message, current_record_name_index + 1)

                resource_record = (name_items, resource_record_type, resource_record_class)

                resource_records_list.append(resource_record)

                current_index = current_record_name_index + 5

            # zona de resposta

            resource_record_serialized = self._serialize_resource_record(resource_records_list[0])

            answer = "\x06google\x03com\0" + struct.pack("!HHIH", 1, 1, 0x10000, 4) + "\xd1\x55\xe5\x93"

            reponse = struct.pack("!HHHHHH", transaction_id, 0x8180, number_questions, 1, 0, 0) + resource_record_serialized + answer

            self.dns_socket.sendto(reponse, address)

    def _serialize_resource_record(self, resource_record):
        """
        Serializes the given resource record into the
        dns binary format.

        @type resource_record: Tuple
        @param resource_record: A tuple with the resource record information.
        @rtype: String
        @return: The string containing the resource record.
        """

        name_items, resource_record_type, resource_record_class = resource_record
        ...
        resource_record_data = struct.pack("!HH", resource_record_type, resource_record_class)

        string_buffer.write(resource_record_data)

        resource_record_serialized = string_buffer.get_value()

        return resource_record_serialized

#            try:
#                # accepts the connection retrieving the dns connection object and the address
#                dns_connection, dns_address = self.dns_socket.accept()
#
#                # creates a new dns client service task, with the given dns connection, address, encoding and encoding handler
#                dns_client_service_task = DnsClientServiceTask(self.main_service_dns_plugin, dns_connection, dns_address, port, encoding, service_configuration, encoding_handler)
#
...
#        self.dns_socket.bind((BIND_HOST_VALUE, port))
#
#        # start listening in the dns socket
...
#
#                    # selects the values
#                    selected_values = select.select([self.dns_socket], [], [], CLIENT_CONNECTION_TIMEOUT)
#
#                # sets the socket to blocking mode
#                self.dns_socket.setblocking(1)
#            except:
...
class DnsClientServiceTask:
    ...
    def send_exception(self, request, exception):
        ...
        # retrieves the dns service error handler plugins
        dns_service_error_handler_plugins = self.main_service_dns_plugin.dns_service_error_handler_plugins
        ...
        for preferred_error_handler in preferred_error_handlers_list:
            ...
            if preferred_error_handler == DEFAULT_VALUE:
                ...
                # iterates over all the dns service error handler plugins
                for dns_service_error_handler_plugin in dns_service_error_handler_plugins:
                    # retrieves the dns service error handler plugin error handler name
                    dns_service_error_handler_plugin_error_handler_name = dns_service_error_handler_plugin.get_error_handler_name()

                    # checks if the plugin is the same as the preferred error handler
                    if dns_service_error_handler_plugin_error_handler_name == preferred_error_handler:
                        # calls the handle error in the dns service error handler plugin
                        dns_service_error_handler_plugin.handle_error(request, exception)
    ...
    def send_request_simple(self, request):
        ...
        try:
            ...
            self.main_service_dns_plugin.error("Problem sending request simple")
    ...
    def send_request_mediated(self, request):
        ...
        try:
            ...
            # error in the client side
            self.main_service_dns_plugin.error("Problem sending request mediated")
        ...
        while True:
            ...
            try:
                ...
                self.dns_connection.send(mediated_value)
            except:
                # error in the client side
                self.main_service_dns_plugin.error("Problem sending request mediated")
    ...
    def send_request_chunked(self, request):
        ...
        try:
            ...
            # error in the client side
            self.main_service_dns_plugin.error("Problem sending request chunked")
        ...
        while True:
            ...
            try:
                ...
                self.dns_connection.send(message_value)
            except:
                # error in the client side
                self.main_service_dns_plugin.error("Problem sending request chunked")
    ...
    def _process_redirection(self, request, service_configuration):
        """
        Processes the redirection stage of the dns request.
    ...
    def _process_handler(self, request, service_configuration):
        """
        Processes the handler stage of the dns request.
    ...
    def _verify_request_information(self, request):
        ...
        if not host_allowed:
            ...
            raise main_service_dns_exceptions.ClientRequestSecurityViolation("host not allowed: " + hostname)
...
class DnsRequest:
    ...
    dns_client_service_task = None
    """ The dns client service task """
    ...
    def __init__(self, dns_client_service_task = None, content_type_charset = DEFAULT_CHARSET):
        self.dns_client_service_task = dns_client_service_task
    ...
    def __setattribute__(self, attribute_name, attribute_value):
        ...
        attribute is the dns request attribute and the setting takes
    ...
    def parse_multipart(self):
        ...
        if not content_type:
            # raises the dns invalid multipart request exception
            raise main_service_dns_exceptions.DnsInvalidMultipartRequestException("no content type defined")
        ...
        if not content_type_value == MULTIPART_FORM_DATA_VALUE:
            # raises the dns invalid multipart request exception
            raise main_service_dns_exceptions.DnsInvalidMultipartRequestException("invalid content type defined: " + content_type_value)
        ...
        if not len(boundary_splitted) == 2:
            # raises the dns invalid multipart request exception
            raise main_service_dns_exceptions.DnsInvalidMultipartRequestException("invalid boundary value: " + boundary)
    ...
    def get_result(self):
        ...
        # writes the dns command in the string buffer (version, status code and status value)
        result.write(self.protocol_version + " " + str(self.status_code) + " " + status_code_value + "\r\n")

        # retrieves the current date time
        current_date_time = datetime.datetime.utcnow()

        # formats the current date time according to the dns specification
        ...
        if self.expiration_timestamp:
            ...
            # formats the expiration date time according to the dns specification
        ...
        if self.last_modified_timestamp:
            ...
            # formats the last modified date time according to the dns specification
    ...
    def verify_resource_modification(self, modified_timestamp = None, etag_value = None):
        ...
        value defined in the dns request.
        ...
        if modified_timestamp and if_modified_header:
            try:
                ...
                self.dns_client_service_task.main_service_dns_plugin.warning("Problem while checking modification date")
    ...
    def _parse_content_disposition(self, headers_map):
        ...
        if not content_disposition:
            # raises the dns invalid multipart request exception
            raise main_service_dns_exceptions.DnsInvalidMultipartRequestException("missing content disposition in multipart value")
        ...
        for content_disposition_attribute in content_disposition_attributes:
            ...
            # invalid state
                ...
                # raises the dns invalid multipart request exception
                raise main_service_dns_exceptions.DnsInvalidMultipartRequestException("invalid content disposition value in multipart value: " + content_disposition_attribute_stripped)
