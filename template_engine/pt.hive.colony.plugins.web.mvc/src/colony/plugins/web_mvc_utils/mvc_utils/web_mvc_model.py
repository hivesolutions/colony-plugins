    ...
    try:
        ...
        for item_name, item_value in map.items():
            ...
            if not hasattr(class_reference, item_name):
                ...
                raise web_mvc_utils_exceptions.ModelApplyException("item name '%s' not found in model class" % item_name)
            ...
            if not class_value_type == types.DictType:
                # raises a model apply exception
                raise web_mvc_utils_exceptions.ModelApplyException("item name '%s' not defined in model class" % item_name)
            ...
            # (cannot change it automatically)
            if value_secure:
                # continues the loop cannot change it
                continue
...
def validate_exception(self, exception_message = VALIDATION_FAILED_MESSAGE, error_description = True):
    ...
    raise web_mvc_utils_exceptions.ModelValidationError(exception_message + ": " + str(model_validation_errors_map), self)
