...
def get_root_class(entity_class, hierarchy):
    """
    Retrieves the class that is to be considered the root of the
    migration, meaning the first concrete class of the hierarchy.

    An abstract class has neither an associated table nor an
    identifier attribute of its own, so it may not be used as the
    reference for the resolution of the table names, of the
    identifier attribute or of the discriminator column.

    :type entity_class: EntityClass
    :param entity_class: The entity class the migration was
    requested for, used as the fallback value.
    :type hierarchy: List
    :param hierarchy: The ordered list of concrete classes of the
    hierarchy, as retrieved by get_hierarchy_classes.
    :rtype: EntityClass
    :return: The class to be used as the root of the migration.
    """
...
def validate_hierarchy(entity_class, target_strategy):
    ...
    # verifies that the entity class has an id field, resolved from
    # the first concrete class as an abstract one has none
    table_id = get_root_class(entity_class, hierarchy).get_id()
    if table_id == None:
...
def generate_concrete_to_cti_queries(
    ...
    root_class = get_root_class(entity_class, hierarchy)
    table_id = root_class.get_id()
    table_id_value = getattr(root_class, table_id)
