...
Always sent: the type, the value and the stack frames of the exception with the surrounding source lines, the logger name, the level, the method, the absolute URL and the status code of the request, the name of its transaction with the numeric identifiers of its path replaced, its headers with the credentials filtered and the query string of the referer removed, the server software and protocol by which it was received, the type of the device derived from its user agent, the environment, the release, the server name, the runtime and operating system contexts, the versions of the installed packages, the version, release, build, modes and start time of the plugin manager, the identifiers of the process and of the thread, the location of the logging call that originated a record, the storage, creation and expire times, locale and attribute names of the session together with a hash of its identifier (never the identifier itself, a credential), and the content type, encoder and elapsed time of the response.

Withheld unless `SENTRY_SEND_USER` is enabled (the default): the address of the client as resolved, every header other than the ones that identify neither the client nor the user (the accept, content, host, origin, referer and user agent headers), as any other header, such as the ones set by a proxy, may carry the address or the identity of the client, the address of the connection, the identifier, username and email of the user, looked up by trial and error in the session and in the user object stored in it, and the values of the session attributes configured in `SENTRY_SESSION_ATTRIBUTES`. The user object and the configured attributes are only read from the values already loaded in them, so that the reporting never triggers the lazy loading of an entity, and only the identifying attributes of an object (its identifier, name, username and email) are sent, truncated as any other value, as the remaining ones may hold personal or tax information.

Withheld unless `SENTRY_SEND_REQUEST` is enabled: the query string and the fields of the request, each value truncated so that a large one (eg: an uploaded file) never leads to the rejection of the event. Even once enabled, every field whose name suggests that it holds a credential, a token, a card, a cookie or a session is replaced by a filtered marker, and the maps and sequences nested under a name that is not itself sensitive are descended into so that the same filtering applies below the top level. The local variables of a frame are never sent, as in a controller they routinely hold entity instances with complete customer records.
