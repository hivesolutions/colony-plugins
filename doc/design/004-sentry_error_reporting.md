...
Withheld unless `SENTRY_SEND_USER` is enabled (the default): the address of the client, both as resolved and as carried by the forwarding headers, the address of the connection, the identifier, username and email of the user, looked up by trial and error in the session and in the user object stored in it, and the values of the session attributes configured in `SENTRY_SESSION_ATTRIBUTES`. The user object and the configured attributes are only read from the values already loaded in them, so that the reporting never triggers the lazy loading of an entity, and only the identifying attributes of an object (its identifier, name, username and email) are sent, as the remaining ones may hold personal or tax information.
