...
class OrganizationalMerchandiseHierarchyTreeNodeVatClassConfiguration:
    ...
    omni OrganizationalMerchandiseHierarchyTreeNodeVatClassConfiguration entities from diamante.
    """
    ...
    def post_conversion_handler_create_organizational_merchandise_hierarchy_tree_node_vat_class_entities(self, data_converter, input_intermediate_structure, output_intermediate_structure, arguments):
        self.omni_migration_diamante_2003_plugin.info("Creating organizational hierarchy merchandise vat class entities")
        ...
        for vat_class_entity in vat_class_entities:
            ...
            if round(vat_class_vat_rate, 2) == VAT_RATE_20:
                vat_20_entity = vat_class_entity

        # skips the handler in case the vat class entity is not found
        if not vat_20_entity:
            self.omni_migration_diamante_2003_plugin.info("Skipping handler because vat class entity was not found")
        ...
        inventory_line_entities = [inventory_line_entity for inventory_line_entity in inventory_line_entities if inventory_line_entity.get_attribute("merchandise") and inventory_line_entity.get_attribute("contactable_organizational_hierarchy_tree_node")]
