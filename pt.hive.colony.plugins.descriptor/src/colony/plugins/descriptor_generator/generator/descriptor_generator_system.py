...
PLUGIN_ID_INSTANCE_IDENTIFIER_PREFIX = "["
""" The plugin id instance id identifier prefix """
...
class DescriptorGenerator:
    ...
    def _generate_plugin_descriptor(self, plugin, template_file_path):
        ...
        plugin_id = PLUGIN_ID_INSTANCE_IDENTIFIER_PREFIX in plugin.id and plugin.id[:plugin.id.index(PLUGIN_ID_INSTANCE_IDENTIFIER_PREFIX)] or plugin.id

        # validate the plugin
        validation_errors = self.descriptor_generator_plugin.validation_plugin_plugin.validate_plugin(plugin_id)
        ...
        plugin_path = self.descriptor_generator_plugin.manager.get_plugin_path_by_id(plugin_id)

        # retrieves the plugin module name
        plugin_module_name = self.descriptor_generator_plugin.manager.get_plugin_module_name_by_id(plugin_id)
    ...
    def _generate_plugin_descriptor_file(self, plugin, plugin_path, plugin_module_name, template_file_path, plugin_system_file_path, plugin_root_directory_path):
        ...
        plugin_descriptor_resources_string = self.__generate_plugin_descriptor_file_resources_string(plugin_resource_file_paths)

        # retrieves the plugin descriptor extra resources
        plugin_descriptor_extra_resources_string = self.__generate_plugin_descriptor_file_extra_resources_string(plugin_resource_file_paths)

        # initializes the plugin descriptor map
        plugin_descriptor_map = {}

        # sets the plugin descriptor attributes
        ...
        plugin_descriptor_map[EXTRA_RESOURCES_VALUE] = plugin_descriptor_extra_resources_string
    ...
    def __generate_plugin_descriptor_file_capabilities_allowed_string(self, plugin):
        ...
        # returns the plugin descriptor capabilities allowed string
    ...
    def __generate_plugin_descriptor_file_dependencies_string(self, plugin):
        ...
        # returns the plugin descriptor dependencies string
        return plugin_descriptor_dependencies_string

    def __generate_plugin_descriptor_file_resources_string(self, plugin_resource_file_paths):
        # creates a string version of the resources attribute
        plugin_descriptor_resources_string = self.stringify_attribute(plugin_resource_file_paths)

        # returns the plugin descriptor resources string
        return plugin_descriptor_resources_string

    def __generate_plugin_descriptor_file_extra_resources_string(self, plugin_resource_file_paths):
        # retrieves the plugin descriptor extra resources
        plugin_descriptor_extra_resources = [PYC_FILE_FORMAT % plugin_resource_file_path for plugin_resource_file_path in plugin_resource_file_paths if plugin_resource_file_path.endswith(PYTHON_FILE_EXTENSION)]

        # creates a string version of the extra resources attribute
        plugin_descriptor_extra_resources_string = self.stringify_attribute(plugin_descriptor_extra_resources)

        # returns the plugin descriptor extra resources string
        return plugin_descriptor_extra_resources_string
    ...
    def get_plugin_resource_file_paths(self, plugin_path, plugin_module_name, plugin_system_file_path, plugin_root_directory_path):
        ...
        file_paths = self.get_file_paths(plugin_system_file_path)
        ...
        for file_path in file_paths:
            ...
            if file in RESOURCE_FILE_NAME_EXCLUSION_LIST or file_extension in RESOURCE_FILE_EXTENSION_EXCLUSION_LIST:
                continue
        ...
        # returns the resource file paths
    ...
    def get_plugin_root_directory_path(self, plugin_system_file_path):
        ...
        # retrieves the number of plugin system file path tokens
        number_plugin_system_file_path_tokens = len(plugin_system_file_path_tokens)

        # tokenizes the plugin system file path with the windows directory separator
        if number_plugin_system_file_path_tokens == 1:
            plugin_system_file_path_tokens = plugin_system_file_path.split(WINDOWS_DIRECTORY_SEPARATOR)

        # retrieves the plugins tokens index
        plugins_token_index = plugin_system_file_path_tokens.index(PLUGINS_VALUE)

        # retrieves the plugin directory root path
        plugin_directory_root_path = plugin_system_file_path_tokens[plugins_token_index + 1]

        # returns the plugin directory root path
        return plugin_directory_root_path

    def get_plugin_system_file_path(self, plugin_path, plugin_module_name):
        # retrieves the plugin module name ending length
        plugin_module_name_ending_length = len(PLUGIN_MODULE_NAME_ENDING)

        # retrieves the base plugin module name
        base_plugin_module_name = plugin_module_name[:-1 * plugin_module_name_ending_length]

        # retrieves the plugin system file name
        plugin_system_file_name = base_plugin_module_name + SYSTEM_FILE_NAME_ENDING

        # retrieves the file paths
        file_paths = self.get_file_paths(plugin_path)
        ...
        for file_path in file_paths:
            ...
            # in case the file is the system file
