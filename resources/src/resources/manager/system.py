...
class ResourcesManager(colony.base.system.System):
    ...
    resources_parser_plugins_map = {}
    """ The resources parser plugins map """
    ...
    def parse_file(self, file_path, full_resources_path):
        ...
        resources_file_parser = parser.ResourcesFileParser(file_path)
    ...
    def update_pending_parses(self):
        """
        Updates the current resources manager state so that
    ...
    def register_resources(self, resources_list, file_path, full_resources_path):
        ...
        try:
            ...
            for resource in resources_list:
                ...
                if resource.__class__ == parser.Validation:
                    # continues the loop
                    continue

                # in case the resource is of type plugin configuration
                if resource.__class__ == parser.PluginConfiguration:
    ...
    def parse_resource_data(self, resource):
        ...
        # map of resources parser plugins
        elif resource_type in self.resources_parser_plugins_map:
            # retrieves the resources parser plugin associated with the
            # resource type
            resources_parser_plugin = self.resources_parser_plugins_map[resource_type]

            # parses the resource with the resources parser plugin
            resources_parser_plugin.parse_resource(resource)
        ...
        # until a proper resources parser plugin is loaded
        else:
            # sets the parse resource data handler, this technique
            # allows a lazy loading of the resources parser plugins
    ...
    def get_system_information(self):
        ...
        # sets the resources manager base values
        resources_manager_base_item["type"] = "map"
        resources_manager_base_item["columns"] = resources_manager_base_item_columns
    ...
    def unload_resources_parser_plugin(self, resources_parser_plugin):
        resources_parser_name = resources_parser_plugin.get_resources_parser_name()

        del self.resources_parser_plugins_map[resources_parser_name]
