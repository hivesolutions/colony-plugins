...
class DataConverterIntermediateStructureIoAdapterEntityManagerPlugin(colony.plugins.plugin_system.Plugin):
    """
    Provides a means to load and save the intermediate structure by using the colony business entity manager.
    """

    id = "pt.hive.colony.plugins.data_converter.intermediate_structure.io_adapter.entity_manager"
    name = "Data Converter Intermediate Structure Input Output Adapter Entity Manager plugin"
    short_name = "Data Converter Intermediate Structure Input Output Adapter Entity Manager"
    description = "Provides a means to load and save the intermediate structure by using the colony business entity manager"
    version = "1.0.0"
    author = "Hive Solutions Lda. <development@hive.pt>"
    loading_type = colony.plugins.plugin_system.EAGER_LOADING_TYPE
    platforms = [colony.plugins.plugin_system.CPYTHON_ENVIRONMENT]
    capabilities = ["data_converter_intermediate_structure_io_adapter.entity_manager"]
    ...
    def load_plugin(self):
        ...
        global data_converter_intermediate_structure_io_adapter_entity_manager
        import data_converter_intermediate_structure_io_adapter_entity_manager.io_adapter_entity_manager.io_adapter_entity_manager_system
        self.io_adapter_entity_manager = data_converter_intermediate_structure_io_adapter_entity_manager.io_adapter_entity_manager.io_adapter_entity_manager_system.IoAdapterEntityManager(self)
    ...
    @colony.plugins.decorators.load_allowed("pt.hive.colony.plugins.data_converter.intermediate_structure.io_adapter.entity_manager", "1.0.0")
    def load_allowed(self, plugin, capability):
        colony.plugins.plugin_system.Plugin.load_allowed(self, plugin, capability)

    @colony.plugins.decorators.unload_allowed("pt.hive.colony.plugins.data_converter.intermediate_structure.io_adapter.entity_manager", "1.0.0")
    def unload_allowed(self, plugin, capability):
        colony.plugins.plugin_system.Plugin.unload_allowed(self, plugin, capability)

    @colony.plugins.decorators.inject_dependencies("pt.hive.colony.plugins.data_converter.intermediate_structure.io_adapter.entity_manager", "1.0.0")
    def dependency_injected(self, plugin):
        colony.plugins.plugin_system.Plugin.dependency_injected(self, plugin)

    def load(self, intermediate_structure, options):
        """
        Populates the intermediate structure with data retrieved from the entity manager source specified in the options.
        ...
        return self.io_adapter_entity_manager.load(intermediate_structure, options)
    ...
    def save(self, intermediate_structure, options):
        ...
        return self.io_adapter_entity_manager.save(intermediate_structure, options)
