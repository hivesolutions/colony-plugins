...
class InternalStructureVisualizerPanel(misc_gui.tree_visualizer.tree_visualizer_system.TreeVisualizerPanel):
    ...
    def add_entity_list(self, node, entity_name_list):
        list_node = self.add_item(node, entity_name_list, 0)
        entity_list = getattr(self.internal_structure, entity_name_list)
        x = 0
        for entity in entity_list:
            entity_node = self.add_item(list_node, entity_name_list + "[" + str(x) + "]", 0)
    ...
    def refresh_tree(self):
        ...
        entity_name_lists = self.get_valid_attributes(self.internal_structure)
        for entity_name_list in entity_name_lists:
            self.add_entity_list(self.tree.GetRootItem(), entity_name_list)
